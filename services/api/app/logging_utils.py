...
def update_daily_rollup(daily_path: str, call: Dict[str, Any]) -> None:
    ...
    if day not in daily:
        daily[day] = {"totals": {"calls": 0, "tokens_in": 0, "tokens_out": 0,
                                 "cost_usd": 0.0, "latency_ms_sum": 0, "cache_hits": 0},
                      "by_model": {}}

    if model not in daily[day]["by_model"]:
        daily[day]["by_model"][model] = {"calls": 0, "tokens_in": 0, "tokens_out": 0,
                                         "cost_usd": 0.0, "latency_ms_sum": 0, "cache_hits": 0}
    ...
    def add(acc, call):
        ...
        if call.get("cache_hit"): acc["cache_hits"] += 1
